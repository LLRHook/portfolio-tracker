...
export default function CsvUpload({ onSuccess }) {
  ...
  const handleFile = useCallback((f) => {
    ...
    reader.onload = (e) => {
      ...
      if (rows.length === 0) { setError('No valid holdings found.'); setPreview([]); }
  ...
  return (
    <div className="flex min-h-[60vh] items-center justify-center">
      <div className="glass glow-cyan w-full max-w-2xl rounded-2xl p-8">
        <div className="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-2xl bg-cyan-500/20">
        ...
        <h2 className="mb-2 text-center text-xl font-semibold text-white">Import Holdings</h2>
        <p className="mb-6 text-center text-sm text-slate-400">
          Export your holdings from Fidelity as CSV and upload here.
        </p>

        {error && (
          <div className="mb-4 rounded-lg bg-rose-500/10 border border-rose-500/20 p-3 text-sm text-rose-400">{error}</div>
        )}
        ...
        <div
          ...
          className={`mb-6 flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed p-8 transition ${
            dragOver ? 'border-cyan-400 bg-cyan-500/10' : 'border-white/10 bg-white/[0.02] hover:border-cyan-500/30 hover:bg-white/[0.04]'
          }`}
        >
          <svg className="mb-3 h-10 w-10 text-slate-500" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
              ...
              <table className="min-w-full divide-y divide-white/5 text-sm">
                ...
                <tbody className="divide-y divide-white/5">
                  {preview.slice(0, 10).map((row, i) => (
                    <tr key={i}>
                      <td className="px-3 py-2 font-medium text-cyan-400">{row.symbol}</td>
                      <td className="px-3 py-2 text-slate-300">{row.description}</td>
                      <td className="px-3 py-2 text-right text-slate-200">{row.quantity}</td>
                      <td className="px-3 py-2 text-right text-slate-200">{row.costBasis}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              {preview.length > 10 && (
                <p className="px-3 py-2 text-xs text-slate-500">...and {preview.length - 10} more rows</p>
              )}
            </div>
            <button
              onClick={handleImport}
              disabled={uploading}
              className="w-full rounded-lg bg-cyan-600 px-4 py-3 text-sm font-medium text-white transition hover:bg-cyan-500 disabled:opacity-50 shadow-lg shadow-cyan-500/25"
            >
